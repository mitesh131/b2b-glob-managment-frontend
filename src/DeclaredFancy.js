...
export default function DeclaredFancy(props) {
    ...
    const submitte = () => {
        ...
        if (num1) {
            if (run) {
                axios.post('https://liveapi247.live/api4/FancyDeclare', {
                    sid: ssid,
                    pass: num1,
                    scoredRun: run,
                    eventId: event_id,
            ...
            props.setFancyRefresh(false);
            toast.success('Must Enter Password to decleard..', { position: toast.POSITION.TOP_CENTER });
        }
        props.DeclareModfancy(false);
                        ...
                        <div className="biab_modal-dialog" id="dialogpop">
                            <div className="biab_modal-content js-modal-content">
                                <div className="biab_modal-header js-modal-header">
                                    <button onClick={() => { props.DeclareModfancy(false) }} type="button" className="biab_close js-close" data-dismiss="modal" aria-label="Close">
                                        <i class="fas fa-times"></i></button>
                                    <h4 className="biab_modal-title" style={{ fontSize: '15px', fontWeight: 'bold' }}>{props.Fancy_Name}&nbsp;=&nbsp;&nbsp;<span style={{ fontSize: '15px', fontWeight: 'bold', color: 'black' }}>{run}</span></h4>
                                </div>
